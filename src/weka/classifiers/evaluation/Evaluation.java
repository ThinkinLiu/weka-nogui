    ...
    try {
      ...
      if ((objectInputFileName.length() != 0) && (objectInputFileName.endsWith(".xml"))) {
        try { // Try to load scheme-specific options as XMLClassifier
          ...
          // 数组复制
          String[] tmp = new String[cl.getOptions().length + options.length];
          System.arraycopy(cl.getOptions(), 0, tmp, 0, cl.getOptions().length);
          System.arraycopy(options, 0, tmp, cl.getOptions().length, options.length);
          options = tmp;
          //1.8才支持Stream故使用上面代码代替options = Stream.concat(Arrays.stream(cl.getOptions()), Arrays.stream(options)).toArray(String[]::new);
      ...
      if (objectInputFileName.length() != 0) {
        ...
        if (objectInputFileName.endsWith(".xml")) {
          try { // Try to load scheme-specific options as XMLClassifier
            ...
            System.arraycopy(options, 0, tmp, cl.getOptions().length, options.length);
            options = tmp;
            //1.8才支持Stream故使用上面代码代替 options = Stream.concat(Arrays.stream(cl.getOptions()), Arrays.stream(options)).toArray(String[]::new);
            objectInputFileName = ""; // We have not actually read a built model, only some options
